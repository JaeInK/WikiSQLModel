...
#class wiki():
    ...
    def build_model(self):
        ...
        # [batch, max_c_len]
        self.contexts = tf.placeholder(shape=[None, 60], dtype=tf.int32, name="context")
        ...
        # [batch, max_q_len]
        self.questions = tf.placeholder(shape=[None, 70], dtype=tf.int32, name="q")
        self.question_legnths = tf.placeholder(shape=[None], dtype=tf.int32, name="q_len")
        
        # [batch, 2, 40]
        # [batch, num_sentences, num_words]
        self.sentences = tf.placeholder(shape=[None, 2, 40], dtype=tf.int32, name="sentences")
        
        # [num_sentences, num_words] = 2, 40
        self.sequence_lengths = tf.placeholder(shape=[2, 40], dtype=tf.int32, name="seq_len")
        # [num_sentences] = 2
        self.sentence_lengths = tf.placeholder(shape=[2], dtype=tf.int32, name="sent_len")
        ...
        print(self.embedded_sentences)
        print(self.embedded_context)
        print(self.embedded_questions)
        ...
        with tf.variable_scope("Embedding_Encoder_Layer"):
            ...
            # [80, batch, filtersize]
            encoded_sentence = self.residual_block(reshaped_sentences, sentence_len, num_blocks=7,
                                                    num_conv_blocks=2, kernel_size=5, num_filters=128, 
                                                    scope="Embedding_Encoder", reuse=True)
            print("\n\n",reshaped_sentences, "\n\n")
            print("\n\n",sentence_len, "\n\n")
